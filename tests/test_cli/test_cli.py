...
def test_main_generation(mock_send_lrs, mock_save_json, mock_send_nextcloud,
                         mock_convert, mock_parser_md, cli_instance, temp_plan_file, tmp_path):
    """Successful generation of presentation."""
    ...
    output_dir.mkdir()
    with patch.object(sys, 'argv', ['script', '--plan', str(temp_plan_file), '--output', str(output_dir), '--format', 'both']):
        ret = cli_instance.main()
        assert ret == 0
    ...
    assert "plan" in local_files and "metadata" in local_files
    assert "pdf" in local_files and "pptx" in local_files
...
@patch('src.__main__.save_json_metadata')
@patch('src.__main__.send_lrs')
def test_process_update_mode(mock_send_lrs, mock_save_json, mock_send_nextcloud,
                             mock_convert, cli_instance, update_directory):
    """Update mode should regenerate presentations and upload files."""
    ret = cli_instance.process_update_mode(update_directory, "both")
    assert ret == 0
        ...
        with patch('src.__main__.send_next_cloud') as mock_send_nc:
            with patch('src.__main__.save_json_metadata') as mock_save:
                with patch('src.__main__.send_lrs'):
                    ret = cli_instance.process_update_mode(update_dir, "both")
                    assert ret == 0
                    saved_xapi = mock_save.call_args[0][0]
                    assert saved_xapi["context"]["extensions"]["plan_url"] == ""
                    assert saved_xapi["context"]["extensions"]["slides_url_pdf"] == ""
                    assert saved_xapi["context"]["extensions"]["slides_url_pptx"] == ""
